					...
					$.getJSON("//" + global.host + "/" + formattedIndex + "_segments/", function(data) {
						...
						$.each(data.indices[index].shards, function (shardKey, shardValue) {
							$.each(shardValue, function(shardKeyPR, shardValuePR) {
								var divId;
								for (var nodeName in global.nodes) {
									var shards = global.nodes[nodeName];
									for (var i = 0; i < shards.length; i++) {
										if (shards[i].node === shardValuePR.routing.node) {
											divId = 'node_' + nodeName + '_' + shardKey;
										}
									}
								}
				...
				.then(function(state) {
					...
					$.each(state.routing_table.indices[targetIndex].shards, function (shardId, shard) {
						$.each(shard, function(prId, pr) {
							...
							if (typeof global.nodes[nodeName] === 'undefined') {
								global.nodes[nodeName] = new Array();
							}

							//css ids can't start with numbers, prepend with "node"
							global.nodes[nodeName].push({
								id: "node_" + nodeName + "_" + shardId,
								node: pr.node,
								index: pr.index,
								primary: pr.primary
							});
					...
					context.nodes = global.nodes;
					return context;
				})
				.render(global.graphTemplate)
				.replace("#content")
				.then(function() {
					$.each(global.nodes, function (nodeId,node) {
